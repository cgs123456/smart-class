...
public class PaymentOrderServiceImpl extends ServiceImpl<PaymentOrderMapper, PaymentOrder>
    ...
    public PaymentOrderVO createOrder(Long userId, PaymentOrderCreateRequest request) {
        ...
        // 生成订单号
        // P2-7: 使用 MyBatis-Plus 雪花算法 IdWorker 替代 currentTimeMillis + 随机数，避免高并发碰撞
    ...
    public PaymentOrderVO payOrder(Long userId, String orderNo) {
        ...
        // 模拟支付
        // P2-8: 此处为模拟支付实现，仅适用于 dev/test 环境。
        // 生产环境必须对接真实支付渠道（微信支付/支付宝），通过 handleCallbackWithVerify
        // 在回调验签通过后再更新订单状态，禁止在生产环境直接调用本方法置为已支付。
        order.setStatus(1);
        ...
        if (product != null && "vip".equals(product.getType())) {
            ...
            // P1-13: 跨服务写（订单状态更新 + VIP 激活）缺乏分布式事务保障，
            // 后续应接入 Seata AT/TCC 或基于 MQ 的最终一致性方案（本地消息表/事务消息），
            // 避免出现订单已支付但 VIP 未激活（或反之）的不一致状态。
        }
        return convertToVO(order);
    }
    ...
    private void verifyWechatCallback(Map<String, String> params, Map<String, String> headers) {
        ...
        // 拼接待签名串：timestamp\nnonce\nbody\n
        // 这里 body 使用 params 中携带的原始信息拼接（实际生产建议使用原始 request body 字节）
    ...
    private boolean doCallbackBusiness(Map<String, String> params) {
        ...
        Product product = productService.getById(order.getProductId());
        if (product != null && "vip".equals(product.getType())) {
            log.info("VIP 商品支付成功，激活会员，userId: {}, level: {}",
                    order.getUserId(), product.getLevel());
            // 实际项目中通过 Feign 调用 user 服务激活 VIP
            // P1-13: 跨服务写（订单状态更新 + VIP 激活）缺乏分布式事务保障，
            // 后续应接入 Seata 或基于 MQ 的最终一致性方案，避免数据不一致。
        }
        return true;
    }
